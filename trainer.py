...
class Trainer:
    ...
    def compute_G_loss(self, batch: Dict) -> torch.Tensor:
        ...
        Computes the generator loss (G_loss).
            1. Zeros the generator optimizer gradients
            2. Computes and returns the generator loss (-1) * D(G(z), z, class_id)

        :param batch: An input batch of data from the dataloader.
        :returns: G_loss, the generator loss averaged over the batch.
        """
        set_requires_grad(self.generator, True)  # Make sure gradients are tracked for this model
        ...
        set_requires_grad(self.encoder, False)  # Do not track gradients back into the encoder
    ...
    def compute_E_loss(self, batch: Dict) -> torch.Tensor:
        ...
        Computes the encoder loss (E_loss).
            ...
            2. Computes and returns the encoder loss (-1) * D(x_real, E(x_real), class_id)
    ...
    def compute_D_loss(self, batch: Dict) -> torch.Tensor:
        ...
        Computes the discriminator loss (D_loss).
            1. Zeros the discriminator optimizer gradients
            2. Computes and returns the discriminator loss:
                D(G(z), z, class_id) - D(x_real, E(x_real), class_id)

        :param batch: An input batch of data from the dataloader.
        :returns: D_loss, the discriminator loss averaged over the batch.
        """
        set_requires_grad(self.discriminator, True)  # Make sure gradients are tracked for this model
        ...
        with torch.no_grad():  # For the discriminator loss, no gradients to other models needed
            x_fake = self.generator(z, class_id)  # Generate fake images (B, 3, 64, 64)
            z_pred = self.encoder(x_real, class_id)  # Encoder z prediction (B, z_dim)
    ...
    def pretrain(self, new_lr: float = None) -> None:
        """
        Runs pre-training on the Generator and Encoder models using an auto-encoder reconstruction loss
        approach to pre-train these models to a baseline level of performance before introducing the full
         adversarial training paradigm by adding the discriminator.
        ...
        with tqdm(initial=self.step, total=self.num_steps) as pbar:
            while self.step < self.num_steps:  # Run all pre-training iterations
                ...
                for model_name in ["generator", "encoder"]:
                ...
                x_hat = self.generator(z_noisy, class_id)  # Generate reconstructions (B, 3, 128, 128)
                recon_loss = F.l1_loss(x_hat, x_real)

                # Compute z_cycle = E(G(z)) with an L2 loss vs the original z vector
                batch_size = len(x_real)  # Will be <= self.batch_size
                z = torch.randn(batch_size, self.z_dim, device=self.device)  # (B, z_dim)
                z_cycle = self.encoder(self.generator(z, class_id), class_id)  # (B, z_dim)
                ...
                loss = (0.1 * prior_loss) + (0.1 * mmd_loss) + (5.0 * recon_loss) + (5.0 * latent_cycle_loss)
                ...
                del batch, x_real, class_id, z_pred, z, prior_loss, recon_loss
    ...
    def generate_samples(self, pretrain: bool = False, seed: int = None):
        """
        Runs G(z) to create a grid of images, 1 for each class, or 10 images if num_classes == 0.
        Results are saved to disk, this method returns None.
        """
        self.generator.eval()
    ...
    def run_eval(self):
        ...
        for batch in self.val_dataloader:
            ...
            x_fake = self.generator(z, class_id)  # Generate fake images (B, 3, 128, 128)
            D_loss_real = self.discriminator(x_real, z_pred, class_id).mean().item()
            D_loss_fake = self.discriminator(x_fake, z, class_id).mean().item()
